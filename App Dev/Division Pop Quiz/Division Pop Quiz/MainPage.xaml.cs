...
namespace Division_Pop_Quiz
{
    /// <summary>
    /// A basic page that provides characteristics common to most applications.
    /// </summary>
    public sealed partial class MainPage : Division_Pop_Quiz.Common.LayoutAwarePage
    {
        int TIME = 300;
        private int counter;
        ...
        TextBlock[,] text;
        TextBox[,] box;
        KeyValuePair<double, double>[,] prob;
        double[,] ans;
        double numCorrect;
        ...
        void setLink()
        {
            ...
            prob = new KeyValuePair<double, double>[4, 5];
        ...
        void setup()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    num1 = (double)random.Next(MIN, MAX + 1);
                    num2 = (double)random.Next(MIN, MAX + 1);
                    prob[i, j] = new KeyValuePair<double, double>(num1, num2);
                    ans[i, j] = num1 / num2;
        ...
        async void checkAns()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    try
                    {
                        result = Convert.ToDouble(box[i, j].Text);
                        if (result <= ans[i, j] + 0.001 && result >= ans[i, j] - 0.001)
                        {
